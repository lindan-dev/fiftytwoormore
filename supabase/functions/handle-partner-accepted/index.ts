...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      ...
      subject: `${receiverProfile?.name || 'Your partner'} just joined 52 or More! 🎉`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #333;">Great news, ${senderProfile?.name || 'there'}!</h1>
          <p style="font-size: 16px; line-height: 1.6; color: #555;">
            <strong>${receiverProfile?.name || 'Your partner'}</strong> has accepted your invitation and joined 52 or More!
          </p>
