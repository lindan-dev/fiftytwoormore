...
// ============ Main Handler (Manual - bypasses time check) ============
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(`Starting MANUAL activation email job... dryRun=${dryRun}, targetUserIds=${targetUserIds?.join(',') || 'all'}`);
    ...
    // Filter out test users and optionally filter to target users
    let profiles = allProfiles?.filter(p => !testUserIds.has(p.user_id)) || [];
    if (targetUserIds) {
      profiles = profiles.filter(p => targetUserIds.includes(p.user_id));
    }
    console.log(`Found ${profiles.length} eligible profiles`);
    ...
    console.log(`Manual activation email job complete. Sent ${emailsSent} emails.`);
    ...
    console.error("Error in send-activation-emails-manual:", error);
